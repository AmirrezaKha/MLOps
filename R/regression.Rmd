...
# Ensure 'model' is a factor and drop any unused levels
vwcars$model <- as.factor(vwcars$model)
vwcars$model <- droplevels(vwcars$model)

# Optional: Set a specific reference level for 'model' (e.g., "ModelA" as reference)
# vwcars$model <- relevel(vwcars$model, ref = "ModelA")
...
```{r}
# Best Subset Selection for all subsets of predictors
best_subsets <- regsubsets(logprice ~ model + age + lp100 + fuelType + engineSize + tax + transmission + mileage, 
                           data = vwcars, nvmax = NULL)

# Create a summary object
subsets_summary <- summary(best_subsets)

# Compute AIC and BIC values for each model subset
aic_values <- numeric(length(subsets_summary$outmat))
bic_values <- numeric(length(subsets_summary$outmat))

for (i in 1:nrow(subsets_summary$outmat)) {
