...
aic_values <- sapply(1:length(subsets_summary$which), function(i) {
  fit <- lm(logprice ~ ., data = as.data.frame(vwcars)[, c(TRUE, subsets_summary$which[i, -1])])
  AIC(fit)
})

bic_values <- sapply(1:length(subsets_summary$which), function(i) {
  fit <- lm(logprice ~ ., data = as.data.frame(vwcars)[, c(TRUE, subsets_summary$which[i, -1])])
  BIC(fit)
})
